...
\emph{Exactly one per \func{make\_decision()} (except \func{rotate}, which is free).}
...
\midrule
\endhead
\func{move(amount)} & Move forward \code{amount} (negative = backward), capped at \code{config.MAX\_MOVE\_SPEED}; stops just before any collision \\
\func{rotate(new\_direction)} & Face \code{new\_direction}\\
\func{pickup(box\_id)} & Pick up an accessible box (fails if too far or hands full) \\
\func{set\_color(box\_id, color)} & Recolor an accessible box to 0 or 1 \\
\func{lay\_down(box\_id)} & Place a held box in front of the robot (fails if blocked / out of bounds) \\
\bottomrule
\end{longtable}
