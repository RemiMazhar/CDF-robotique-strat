...
\begin{itemize}
    ...
    \item \textbf{Cooldown / temporal cost}: \func{move}, \func{pickup},
          ...
          \code{config.MOVE\_COOLDOWN}, \code{config.PICKUP\_COOLDOWN},
          \code{config.SET\_COLOR\_COOLDOWN} and
          \code{config.LAY\_DOWN\_COOLDOWN} respectively. A value of $N$ means
...
\func{get\_obstacle\_distance(player)} & Distance from \code{player}'s robot to the nearest obstacle (map edge, other robot, or laid-down box) along its current orientation; \textbf{not} capped at \code{config.MAX\_MOVE\_SPEED}, may be \code{inf} \\
...
\midrule
\endhead
\func{move(amount)} & Move forward by \code{amount} ($\geq 0$; capped at \code{config.MAX\_MOVE\_SPEED}); stops just before any collision. Starts a cooldown of \code{config.MOVE\_COOLDOWN} ticks \\
\func{rotate(new\_direction)} & Face \code{new\_direction}. Does not itself start a cooldown\\
\func{pickup(box\_id)} & Pick up an accessible box (fails if too far or hands full). Starts a cooldown of \code{config.PICKUP\_COOLDOWN} ticks \\
\func{set\_color(box\_id, color)} & Recolor an accessible box to 0 or 1. Starts a cooldown of \code{config.SET\_COLOR\_COOLDOWN} ticks \\
\func{lay\_down(box\_id)} & Place a held box in front of the robot (fails if blocked / out of bounds). Starts a cooldown of \code{config.LAY\_DOWN\_COOLDOWN} ticks \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\code{config.MOVE\_COOLDOWN} & 2 & Total ticks \func{move} occupies the robot
    for, including the tick it succeeds on. \\
\code{config.PICKUP\_COOLDOWN} & 5 & Total ticks \func{pickup} occupies the
    robot for, including the tick it succeeds on. \\
\code{config.SET\_COLOR\_COOLDOWN} & 5 & Total ticks \func{set\_color}
    occupies the robot for, including the tick it succeeds on. \\
\code{config.LAY\_DOWN\_COOLDOWN} & 5 & Total ticks \func{lay\_down} occupies
