...
\begin{description}[leftmargin=1.2cm,style=nextline]
    ...
    \item[Robots] Chaque joueur contrôle un robot circulaire qui démarre
        dans sa propre zone. Un robot peut tourner librement, avancer,
        ramasser une caisse proche, changer la couleur d'une caisse proche, ou
        poser une caisse qu'il transporte.
        Avancer, ramasser, recolorer et poser prennent du temps réel : ces
        actions placent le robot en période de \emph{cooldown}
        pendant laquelle il ne peut plus agir du tout (voir la
        Section~\ref{sec:one-action}).
    ...
    \item[Score] Calculé à la fin de la partie :
        \begin{itemize}[nosep]
            \item chaque caisse posée dans la zone du joueur $X$
                  fait gagner \textbf{+\code{POINTS\_OWN\_AREA}} points à
                  $X$ (quelle que soit la couleur de la caisse) ;
            \item chaque caisse posée dans une zone de score et dont la
                  couleur est $X$ fait gagner
                  \textbf{+\code{POINTS\_SCORING\_AREA}} points à $X$ ;
...
\begin{itemize}
    \item \textbf{Une action par tour} : \func{make\_decision()} peut
          ...
          fois à chaque tour (sous réserve de la règle de coût temporel
          ci-dessous).
    \item \textbf{Emplacement de pose} : à une distance fixe
          (\code{LAY\_DOWN\_DISTANCE}) directement devant le robot,
    ...
    \item \textbf{Disposition initiale} : les positions, orientations et
          zones sont fixes et entièrement définies dans le fichier de
          configuration. Les couleurs des caisses sont également définies
          dans la configuration par défaut, sauf si la partie est lancée
          avec \code{python play.py run --random-colors}, qui attribue alors
          une couleur aléatoire (0 ou 1) à chaque caisse.
    ...
    \item \textbf{Coût temporel} : \func{move},
          \func{pickup}, \func{set\_color} et \func{lay\_down} occupent
          chacune le robot pendant un nombre de ticks donné par leur propre
          constante --- \code{MOVE\_COOLDOWN},
          \code{PICKUP\_COOLDOWN}, \code{SET\_COLOR\_COOLDOWN}
          et \code{LAY\_DOWN\_COOLDOWN} respectivement. Une valeur $N$
          signifie que le robot est occupé pendant $N$ ticks \emph{au total},
          y compris le tick où l'action réussit (donc $N=1$ ne donne aucun
          délai supplémentaire, et $N>1$ donne $N-1$ ticks supplémentaires de
          gel). Tant que le robot est occupé, \textbf{toute} fonction
          d'action --- y compris \func{rotate} --- lève \code{GameError} sans
          consommer le tour. Appelez \func{get\_cooldown(player)} pour connaître
          le nombre de ticks restants (0 = libre).
...
python play.py run --random-colors  # tire au hasard la couleur initiale de chaque caisse
...
\func{get\_obstacle\_distance(player)} & Distance entre le robot de \code{player} et l'obstacle le plus proche (bord de la carte, autre robot, ou caisse posée) dans la direction actuelle ; \textbf{non} limitée par \code{MAX\_MOVE\_SPEED}, peut valoir \code{inf} \\
...
\func{get\_cooldown(player)} & Nombre de ticks restants avant que le robot de \code{player} puisse agir à nouveau (0 = libre maintenant) \\
\bottomrule
\end{longtable}

\subsection{Fonctions d'action}
\emph{Exactement une par appel à \func{make\_decision()} (sauf
\func{rotate}, qui ne compte pas comme une action). Les cinq, y compris
\func{rotate}, sont bloquées tant que le robot est en cooldown}
...
\midrule
\endhead
\func{move(amount)} & Avance de \code{amount} ($\geq 0$ ; plafonné à \code{MAX\_MOVE\_SPEED}) ; s'arrête juste avant toute collision. Déclenche un cooldown de \code{MOVE\_COOLDOWN} ticks \\
\func{rotate(new\_direction)} & Oriente le robot vers \code{new\_direction}. Ne déclenche elle-même aucun cooldown \\
\func{pickup(box\_id)} & Ramasse une caisse accessible (échoue si trop loin ou si les mains sont pleines). Déclenche un cooldown de \code{PICKUP\_COOLDOWN} ticks \\
\func{set\_color(box\_id, color)} & Recolore une caisse accessible en 0 ou 1. Déclenche un cooldown de \code{SET\_COLOR\_COOLDOWN} ticks \\
\func{lay\_down(box\_id)} & Pose une caisse transportée devant le robot (échoue si bloqué / hors limites). Déclenche un cooldown de \code{LAY\_DOWN\_COOLDOWN} ticks \\
\bottomrule
\end{longtable}
...
\begin{itemize}[nosep]
    ...
    \item \code{game.GameError} si l'action elle-même est invalide (hors de
          portée, bloquée, valeur de \func{move} négative, etc.) \textbf{ou si
          le robot est encore en cooldown suite à une action précédente}
          (cela s'applique aussi à \func{rotate}). Cela ne consomme
          \textbf{pas} le tour, vous pouvez donc vous rattraper et essayer
          autre chose.
\end{itemize}
...
\midrule
\endhead
\code{MAP\_WIDTH} & 1000.0 & Largeur du terrain de jeu, en unités de
    jeu (étendue selon \code{x} ; pour rappel, \code{(0, 0)} est le coin
    supérieur gauche). \\
\code{MAP\_HEIGHT} & 600.0 & Hauteur du terrain de jeu (étendue selon
    \code{y}). \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\code{PLAYER0\_AREA} & \scriptsize(0, 200, 150, 200) & Rectangle
    \code{(x, y, largeur, hauteur)} définissant la zone de base du joueur 0. \\
\code{PLAYER1\_AREA} & \scriptsize(850, 200, 150, 200) & Idem, pour le
    joueur 1 (\code{Area.type == 1}). \\
\code{SCORING\_AREAS} & \scriptsize liste de 2 rectangles & Liste de
...
\midrule
\endhead
\code{PLAYER0\_START} & (75, 300) & Position de départ \code{(x, y)}
    du robot du joueur 0 --- à l'intérieur de \code{PLAYER0\_AREA}. \\
\code{PLAYER0\_START\_ANGLE} & 0.0 & Angle d'orientation initial du
    robot du joueur 0, en \textbf{degrés} mesurés depuis l'axe $+x$
    ($0^\circ$ = orienté vers la droite, vers le centre de la carte). \\
\code{PLAYER1\_START} & (925, 300) & Position de départ du robot du
    joueur 1. \\
\code{PLAYER1\_START\_ANGLE} & 180.0 & Angle d'orientation initial du
...
\midrule
\endhead
\code{ROBOT\_RADIUS} & 20.0 & Rayon du corps circulaire du robot. \\
\code{MAX\_MOVE\_SPEED} & 10.0 & La plus grande valeur de
    \code{|amount|} qu'un appel à \func{move} appliquera réellement, par
    tick. \\
\code{MAX\_BOXES\_HELD} & 3 & Nombre maximal de caisses qu'un robot
...
\midrule
\endhead
\code{BOX\_WIDTH} & 40.0 & Largeur d'une caisse. \\
\code{BOX\_HEIGHT} & 25.0 & Hauteur d'une caisse. \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\code{ACCESSIBILITY\_RADIUS} & 25.0 & Distance cercle-rectangle
...
\code{LAY\_DOWN\_DISTANCE} & 45.0 & Distance fixe entre le centre du
...
\midrule
\endhead
\code{POINTS\_OWN\_AREA} & 3 & Points attribués au joueur $X$ pour
    chaque caisse posée dans la zone de $X$. \\
\code{POINTS\_SCORING\_AREA} & 5 & Points attribués au joueur $X$
...
\midrule
\endhead
\code{DT} & 0.1 & Nombre de secondes représentées par un tick. \\
\code{GAME\_DURATION} & 60.0 & Durée totale de la partie, en
    secondes ($=$ \code{DT} $\times$ \code{TOTAL\_TICKS}). \\
\code{TOTAL\_TICKS} & 600 & Nombre de ticks dans une partie. \\
\bottomrule
\end{longtable}

\subsubsection*{Coûts d'action (temporels)}
\begin{longtable}{p{4.7cm}p{2cm}p{8cm}}
\toprule
\textbf{Constante} & \textbf{Valeur} & \textbf{Signification} \\
\midrule
\endhead
\code{MOVE\_COOLDOWN} & 1 & Nombre total de ticks pendant lesquels
    \func{move} occupe le robot, y compris le tick où elle réussit. \\
\code{PICKUP\_COOLDOWN} & 20 & Nombre total de ticks pendant lesquels
    \func{pickup} occupe le robot, y compris le tick où elle réussit. \\
\code{SET\_COLOR\_COOLDOWN} & 10 & Nombre total de ticks pendant
    lesquels \func{set\_color} occupe le robot, y compris le tick où elle
    réussit. \\
\code{LAY\_DOWN\_COOLDOWN} & 20 & Nombre total de ticks pendant
    lesquels \func{lay\_down} occupe le robot, y compris le tick où elle
    réussit. \\
\bottomrule
\end{longtable}

Tant que l'une de ces temporisations est active
(\code{get\_cooldown(player) > 0}), \textbf{toutes} les fonctions d'action ---
y compris \func{rotate} --- lèvent \code{GameError} sans consommer le tour.
...
\midrule
\endhead
\code{INITIAL\_BOXES} & liste de 12 tuples & La disposition de départ
