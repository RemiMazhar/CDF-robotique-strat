...
\begin{itemize}
    ...
    \item \textbf{Coût temporel} : \func{move},
          ...
          constante --- \code{config.MOVE\_COOLDOWN},
          \code{config.PICKUP\_COOLDOWN}, \code{config.SET\_COLOR\_COOLDOWN}
          et \code{config.LAY\_DOWN\_COOLDOWN} respectivement. Une valeur $N$
...
\func{get\_obstacle\_distance(player)} & Distance entre le robot de \code{player} et l'obstacle le plus proche (bord de la carte, autre robot, ou caisse posée) dans la direction actuelle ; \textbf{non} limitée par \code{config.MAX\_MOVE\_SPEED}, peut valoir \code{inf} \\
...
\midrule
\endhead
\func{move(amount)} & Avance de \code{amount} ($\geq 0$ ; plafonné à \code{config.MAX\_MOVE\_SPEED}) ; s'arrête juste avant toute collision. Déclenche un cooldown de \code{config.MOVE\_COOLDOWN} ticks \\
\func{rotate(new\_direction)} & Oriente le robot vers \code{new\_direction}. Ne déclenche elle-même aucun cooldown \\
\func{pickup(box\_id)} & Ramasse une caisse accessible (échoue si trop loin ou si les mains sont pleines). Déclenche un cooldown de \code{config.PICKUP\_COOLDOWN} ticks \\
\func{set\_color(box\_id, color)} & Recolore une caisse accessible en 0 ou 1. Déclenche un cooldown de \code{config.SET\_COLOR\_COOLDOWN} ticks \\
\func{lay\_down(box\_id)} & Pose une caisse transportée devant le robot (échoue si bloqué / hors limites). Déclenche un cooldown de \code{config.LAY\_DOWN\_COOLDOWN} ticks \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\code{config.MOVE\_COOLDOWN} & 2 & Nombre total de ticks pendant lesquels
    \func{move} occupe le robot, y compris le tick où elle réussit. \\
\code{config.PICKUP\_COOLDOWN} & 5 & Nombre total de ticks pendant lesquels
    \func{pickup} occupe le robot, y compris le tick où elle réussit. \\
\code{config.SET\_COLOR\_COOLDOWN} & 5 & Nombre total de ticks pendant
    lesquels \func{set\_color} occupe le robot, y compris le tick où elle
    réussit. \\
\code{config.LAY\_DOWN\_COOLDOWN} & 5 & Nombre total de ticks pendant
