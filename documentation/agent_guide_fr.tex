...
\midrule
\endhead
\func{move(amount)} & Avance de \code{amount} (négatif = recule), plafonné à \code{config.MAX\_MOVE\_SPEED} ; s'arrête juste avant toute collision. Déclenche un cooldown de \code{config.MOVE\_COOLDOWN} ticks \\
...
\begin{itemize}[nosep]
    ...
    \item \code{game.GameError} si l'action elle-même est invalide (hors de
          portée, bloquée, etc.) \textbf{ou si le robot est encore en
          cooldown suite à une action précédente} (cela s'applique aussi
          à \func{rotate}). Cela ne consomme \textbf{pas} le tour, vous pouvez
          donc vous rattraper et essayer autre chose.
\end{itemize}
