...
- **Movement collision** (`is_move_colliding`, used by `do_move`): binary
  search over `COLLISION_STEPS` iterations finds the maximum safe travel
  distance before hitting the map edge, the other robot, or any laid-down box
  — the robot is moved to just-before the obstacle rather than stopped at an
  arbitrary fraction.
